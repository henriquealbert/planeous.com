...
export function TableSort({ data }: TableSortProps) {
      ...
      <td>
        <Group spacing="sm">
          <Avatar
            ...
            src={row.avatar}
            radius="xl"
            sx={{ background: getRandomColorFromString(row.name) }}
          >
            {getNamesInitials(row.name)}
          </Avatar>
  ...
  return (
    <ScrollArea>
      <TableControls handleSearchChange={handleSearchChange} search={search} />
      <Table
        horizontalSpacing="md"
        verticalSpacing="xs"
        sx={{ tableLayout: 'fixed', minWidth: 700, width: '100%' }}
      >
        <thead>
          <tr>
            <Th
